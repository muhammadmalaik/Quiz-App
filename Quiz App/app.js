...
function nextQuestion() {
  ...
  if (index >= questions.length) {
    var scorePercent = ((score / questions.length) * 100).toFixed(2);

    // Show different popups based on score range
    if (scorePercent >= 80) {
      Swal.fire({
        title: "Excellent!",
        text: `Great job! You scored ${scorePercent}%.`,
        icon: "success",
      });
    } else if (scorePercent >= 60) {
      Swal.fire({
        title: "Good!",
        text: `Well done! You scored ${scorePercent}%.`,
        icon: "info",
      });
    } else if (scorePercent >= 40) {
      Swal.fire({
        title: "Fair",
        text: `Not bad! You scored ${scorePercent}%.`,
        icon: "warning",
      });
    } else {
      Swal.fire({
        title: "Keep Trying",
        text: `You scored ${scorePercent}%. Better luck next time!`,
        icon: "error",
      });
    }
  } else {
